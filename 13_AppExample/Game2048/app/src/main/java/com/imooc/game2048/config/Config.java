...
/**
 * 游戏具有配置功能，需要使用一个数据持久化方法（SharedPreferences）保存这些配置
 * 实现一个Application类，保存配置信息，并在Manifest文件中设置为程序的Application入口
 * 可以用来放一些全局的和一些上下文都要用到变量和方法之类
 *
 * 完成对游戏配置文件的读写，并给其增加默认的游戏配置值
 */
