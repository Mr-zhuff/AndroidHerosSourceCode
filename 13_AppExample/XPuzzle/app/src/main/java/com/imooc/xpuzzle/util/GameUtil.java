...
public class GameUtil {
    ...
    /**
     * 是否拼图成功
     *      判断当前图片的Item和初始状态下图片的ID是否相同
     *      空白Item的ID与初始状态最后一个图片的ID相同
     *      全部相同则为成功
     * @return 是否拼图成功
     */
    ...
    public static boolean canSolve(List<Integer> data) {
        ...
        // 可行性原则
        if (data.size() % 2 == 1) { //序列宽度为奇数，倒置变量值的和必须为偶数
            return getInversions(data) % 2 == 0;
        } else {
            // 从底往上数,空格位于奇数行
            if (((blankId - 1) / PuzzleMain.TYPE) % 2 == 1) {
                return getInversions(data) % 2 == 0;  //偶数
            } else {
                // 从底往上数,空位位于偶数行
                return getInversions(data) % 2 == 1;    //奇数
            }
        }
    }
