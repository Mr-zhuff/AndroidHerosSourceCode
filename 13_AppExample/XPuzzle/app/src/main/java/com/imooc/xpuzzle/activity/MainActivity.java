...
public class MainActivity extends Activity implements OnClickListener {
        ...
        mGvPicList.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View view,
                ...
                if (position == mResPicId.length - 1) {
                    ...
                    intent.putExtra("mType", mType);    //传入难度类型
    ...
    /**
     * 使用了简化的builder模式，将抽象建造者省略，只使用一个具体建造者，
     * 并且将指导者角色也省略了。 AlertDialog.Builder就是具体建造者，
     * 另外，它是以匿名类的方式被创建的，而且，Builder类是AlertDialog的内部类，
     * 这样，耦合性比较低
     */
    // 显示选择系统图库 相机对话框
    private void showDialogCustom() {
        AlertDialog.Builder builder = new AlertDialog.Builder(
                MainActivity.this); //弹出对话框
        builder.setTitle("选择：");
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (0 == which) {
                            ...
                            intent.setDataAndType(
                                    ...
                                    IMAGE_TYPE);
                            startActivityForResult(intent, RESULT_IMAGE);   // 返回码
                        } else if (1 == which) {
                            // 系统相机
                            Intent intent = new Intent(
                                    MediaStore.ACTION_IMAGE_CAPTURE);
                            Uri photoUri = Uri.fromFile(
                                    new File(TEMP_IMAGE_PATH)); //照片存放路径
                            intent.putExtra(
                                    MediaStore.EXTRA_OUTPUT,
                                    photoUri);
                            startActivityForResult(intent, RESULT_CAMERA);  //返回码
                        }
                    }
                });
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {  //请求码返回RESULT_OK
            if (requestCode == RESULT_IMAGE && data != null) {  //图库返回码 + data
                // 相册
                Cursor cursor = this.getContentResolver().query(    //查询图片
                ...
                intent.putExtra("mType", mType);    // 难度类型
                cursor.close();
                startActivity(intent);
            } else if (requestCode == RESULT_CAMERA) {  //相机返回码
                // 相机
                Intent intent = new Intent(MainActivity.this, PuzzleMain.class);
    ...
    private void popupShow(View view) {
        ...
        mPopupWindow.showAtLocation(
                ...
                location[1] + 40 * density);    // PopupWindow 显示位置
    }
    ...
    private void initViews() {
        ...
        Bitmap[] bitmaps = new Bitmap[mResPicId.length];    //bitmap数组
        for (int i = 0; i < bitmaps.length; i++) {
            bitmaps[i] = BitmapFactory.decodeResource(
                    getResources(), mResPicId[i]);
            mPicList.add(bitmaps[i]);   // 遍历保存到List中
        }
